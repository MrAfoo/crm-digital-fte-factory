...
async def _process_web_ticket(ticket_id: str, message: str, channel: str, customer_id: str, customer_name: str = "Customer", customer_email: str = "", ticket_subject: str = ""):
    """Background task: run agent on web ticket and store Nova's reply."""
    try:
        if not (hasattr(app.state, "agent") and app.state.agent):
            return
        result = await app.state.agent.run(
            message, channel, customer_id, None,
